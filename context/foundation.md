...
| 4 | **Hosting: Railway** *(changed from Vercel, 2026-08-04, Gold's call)* | Gold chose Railway. Next runs as a long-lived Node server (`next start`, binds `$PORT`); v1 is entirely dynamically-rendered so Vercel's edge/ISR edge isn't in play. `next/og` runs on the Node runtime. **Phase 2 reverses this → back to Vercel** for native wildcard subdomains + programmatic custom-domain/TLS + ISR (`§13 P-Q2`, locked 2026-08-04); v1 stays on Railway until the pre-M7 cutover. | Vercel (original) — better Next edge/ISR, but unused while the menu is force-dynamic |
...
| **P-Q2** | **Hosting — LOCKED 2026-08-04: migrate to Vercel** (from Railway). Cutover **before M7**; M1–M6 are hosting-agnostic, so v1 stays on Railway until then. Supersedes §7 #4. | Vercel is built for this exact case: wildcard subdomains (`*.platter.menu`), a programmatic custom-domain + auto-TLS API, edge middleware for host→venue resolution, ISR/tag-cache for the per-theme perf budget. Railway would need Cloudflare-for-SaaS + a wildcard cert + more DevOps. The move is **cheap now** (standard Next 16 app, Supabase is external), expensive after Phase 2 code. **Note:** Vercel acct is on **Hobby** — Pro needed at M7 for the domain automation, not before. | ✅ **Locked (Gold)** |
